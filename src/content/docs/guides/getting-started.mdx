...
* Something that can compile Swift code. macOS and Linux are
  preferred. Get Swift [here](https://www.swift.org/install/).
...
:::note
On macOS, using Xcode for DDBKit development is
recommended. It offers Swift-oriented features
to make editing code easier. For easier version
management, it is recommended that you use
[Xcodes][xcodes].
:::
...
:::caution[Important]
On macOS and other Apple platforms, you need to specify
minimum deployment versions for macOS 13, iOS 16 etc.
in order for your package to build.
...
intents and some other things. Open
`Sources/MyBot/Bot.swift` and replace the contents with:
...
A universal way of doing so is to run this in your bot's
directory:
...
many ways to do so.
The [template](https://github.com/DDBKit/DDBKit-Template)
provides a Dockerfile and docker-compose.yml if you'd like
to use Docker. Also consider systemd/launchd services.

If you need to compile your bot, run
