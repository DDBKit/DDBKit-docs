...
```
The extension is still recommended to be able to access `bot` and `cache` properties of your bot structure.
:::

### Context Commands

You can also create context menu commands using `ContextCommand`. For example, a message context command:
```swift
...
There will now be an "Explode" option in the context menu when right-clicking messages. You can also make user context commands by setting `kind: .user`.
