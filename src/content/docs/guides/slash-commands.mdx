...
commands which can be categorized. For instance, a
<code type="cmd">/ban</code> command could fall under a
<code type="cmd">/moderation</code> subcommand, and be used
like <code type="cmd">/moderation ban</code>.

Subcommands can also be nested into SubcommandGroups
for a total of 3 labels. This example would produce
a <code type="cmd">/data merge entity</code> command:
