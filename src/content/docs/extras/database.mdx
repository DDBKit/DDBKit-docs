...
<Aside type="danger">
  This database is very basic and not suited for large-scale applications.<br></br>
...
The `transaction` method is used to ensure that the data is not modified by another command while it is being modified by this one.
If another thread tries to modify the same data while the transaction is in progress, it will wait until the existing transaction is complete before modifying the data.
